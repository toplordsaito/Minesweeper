...
class MainGame extends Component {
  ...
  constructor(props) {
    ...
    this.grid = Array.apply(null, Array(this.state.BOARD_SIZE)).map(
      (el, idx) => {
        return Array.apply(null, Array(this.state.BOARD_SIZE)).map(
          (el, idx) => {
            return null;
          }
        );
    ...
    for (let i = -1; i <= 1; i++) {
      for (let j = -1; j <= 1; j++) {
        if (
          (i != 0 || j != 0) &&
          x + i >= 0 &&
          x + i <= this.state.BOARD_SIZE - 1 &&
          y + j >= 0 &&
          y + j <= this.state.BOARD_SIZE - 1
        ) {
  ...
  renderBoard = () => {
    return Array.apply(null, Array(this.state.BOARD_SIZE)).map((el, rowIdx) => {
      let cellList = Array.apply(null, Array(this.state.BOARD_SIZE)).map(
        (el, colIdx) => {
          return (
            <Cell
